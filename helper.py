...
def covered(sheep_pos, sheeps, dog_pos):
    ...
    for sheep_1 in sheeps:
        ...
        angle = math.atan2(dog_sheep_vector[0], dog_sheep_vector[1]) - math.atan2(dog_sheep1_vector[0],
                                                                                  dog_sheep1_vector[1])
...
def left_most_visible_from_sheepfold(sheeps, visibility, goal):
    ...
    left_most_angle = math.atan2(left_most_vector[0], left_most_vector[1]) - math.atan2(initial_vector[0],
                                                                                        initial_vector[1])
    left_most_dist = euclidean(left_most, goal)

    for sheep in visible_sheeps:
        sheep_goal_vector = unit_vector(sheep - goal)
        angle = math.atan2(sheep_goal_vector[0], sheep_goal_vector[1]) - math.atan2(initial_vector[0],
                                                                                    initial_vector[1])
...
def right_most_visible_from_sheepfold(sheeps, visibility, goal):
    ...
    initial_vector = unit_vector(initial_point - goal)
    right_most_angle = math.atan2(right_most_vector[0], right_most_vector[1]) - math.atan2(initial_vector[0],
                                                                                           initial_vector[1])
...
def calculateLC(sheeps, goal, dog_radius, left_right_sheep, dog_pos):
    if left_right_sheep == 'left':
        dog_visibility = visible_sheep(dog_pos, dog_radius, sheeps)
        ...
        return math.acos(
            np.dot(D_cd, left_most_sheep_dog_vector) / (vector_size(D_cd) * vector_size(left_most_sheep_dog_vector)))
        ...
        D_cd = unit_vector(goal - center_of_visible_sheep)
        return math.acos(
            np.dot(D_cd, right_most_sheep_dog_vector) / (vector_size(D_cd) * vector_size(right_most_sheep_dog_vector)))


def visible_sheep(dog_pos, dog_radius, sheeps):
    """
